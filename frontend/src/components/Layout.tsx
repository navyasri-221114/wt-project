...
export default function Layout({ user, setUser }: { user: any, setUser: any }) {
  ...
  if (user?.role === 'student') {
    navItems.push({ label: 'Companies', icon: Building2, path: '/companies' });
  }
      ...
      {!isAdmin && (
        <aside className="w-64 bg-white border-r border-slate-200 flex flex-col h-screen sticky top-0">
          ...
          <div className="p-4 border-t border-slate-200">
            <Link to="/" className="w-full flex items-center gap-3 px-4 py-3 text-sm font-medium text-slate-600 hover:bg-slate-50 rounded-xl transition-colors mb-2">
              <ArrowLeft size={20} />
              Home Page
            </Link>

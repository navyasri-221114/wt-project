...
export default function AuthPage({ setUser }: { setUser: any }) {
  ...
  const login = useGoogleLogin({
    ...
    onError: () => setError('Google sign-in failed. Please make sure your current URL is authorized.')
  });
            ...
            {role === 'student' && (
              <div className="mt-4">
                ...
                <button
                  type="button"
                  onClick={() => login()}
                  className="w-full flex items-center justify-center gap-3 py-3 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-all font-medium text-slate-700 shadow-sm"
                >
